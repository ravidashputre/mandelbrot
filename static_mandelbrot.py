...
class Static_mandelbrot:
    ...
    def window_zoom(self,x_min,x_max,y_min,y_max,iterations,precision):
        self.mandelbrot_x = []
        self.mandelbrot_y = []
        fig = plt.figure()
        #ax = fig.add_subplot(111)
        #plt.axis([x_min,x_max,y_min,y_max])
        for i in np.arange(x_min,x_max+0.01,precision):
            for l in np.arange(x_min,y_max+.01,precision):
                c = complex(i,l)
                if(self.decide_point(c,iterations,100)):
                    self.mandelbrot_x=np.append(self.mandelbrot_x, c.real)
                    self.mandelbrot_y=np.append(self.mandelbrot_y, c.imag)
        return (self.mandelbrot_x,self.mandelbrot_y)
